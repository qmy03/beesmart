...
const AccountPage: React.FC = () => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        if (response?.data?.status === 200) {
          ...
          setEditableInfo({
            fullName: response.data.data.fullName,
            dateOfBirth: formatDate1(response.data.data.dateOfBirth),
            email: response.data.data.email,
          });
        } else {
          console.error("Failed to fetch user info:", response.data.message);
      ...
      const fetchQuizHistory = async () => {
        try {
          ...
          const response = await apiService.get<ApiResponse<QuizRecordsResponse>>(
            `/statistics/user/${userInfo.userId}/quiz-records?page=${page}&size=${rowsPerPage}${subjectQuery}`,
            {
              headers: { Authorization: `Bearer ${accessToken}` },
            }
          );
          if (response?.data?.status === 200) {
            const formattedRecords = response.data.data.quizRecords.map((record) => ({
              ...record,
              timeSpentFormatted: formatTimeSpent(record.timeSpent),
            }));
            setQuizHistory(formattedRecords);
            setTotalItems(response.data.data.totalItems || 0);
          } else {
            console.error("Failed to fetch quiz history:", response.data.message);
          }
        } catch (error) {
          console.error("Error fetching quiz history:", error);
        }
  ...
  }, [activeTab, userInfo, accessToken, page, rowsPerPage, selectedSubject]);
  ...
  const handleUpdateUser = async () => {
    try {
      const updatedInfo = {
        ...editableInfo,
        dateOfBirth: formatDate(editableInfo.dateOfBirth),
      };
      const response = await apiService.put<ApiResponse>(
        "/users/user-info",
        updatedInfo,
  ...
  const handleRegisterStudent = async () => {
    if (!studentInfo.fullName || !studentInfo.username || !studentInfo.password || !studentInfo.grade) {
      setSnackbarMessage("Vui lòng điền đầy đủ thông tin!");
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
      return;
    ...
    try {
      ...
      if (response?.data?.status === 200) {
        setSnackbarMessage(response?.data?.message || "Tạo tài khoản thành công!");
        setSnackbarSeverity("success");
      } else {
        setSnackbarMessage(response?.data?.message || "Tạo tài khoản thất bại!");
        setSnackbarSeverity("error");
      }
    } catch (error: any) {
      const errorMessage =
        error.response?.data?.message === "Username already exists"
          ? "Tên người dùng đã tồn tại! Vui lòng thử tên khác."
          : error.response?.data?.message || "Tạo tài khoản thất bại!";
      setSnackbarMessage(errorMessage);
      setSnackbarSeverity("error");
    } finally {
  ...
  return (
    <Layout>
      ...
      >
        {/* Left Sidebar */}
        <Box sx={{ flex: 1, bgcolor: "white", padding: 2, boxShadow: 4, borderRadius: 2 }}>
          <Box sx={{ textAlign: "center", mb: 3 }}>
            <Avatar
              sx={{
                bgcolor: "#99BC4D",
          ...
          <Box sx={{ mt: 3 }}>
            <Button
              ...
              sx={{
                justifyContent: "flex-start",
                textTransform: "none",
                color: activeTab === "Thông tin cá nhân" ? "#99BC4D" : "#99BC4D",
                bgcolor: activeTab === "Thông tin cá nhân" ? "#FFFBF3" : "white",
                "&:hover": { bgcolor: "#FFFBF3", color: "#99BC4D" },
              }}
            >
              Thông tin cá nhân
            ...
            {userInfo.role === "PARENT" && (
              <Button
                ...
                sx={{
                  justifyContent: "flex-start",
                  textTransform: "none",
                  color: activeTab === "Tạo tài khoản cho con" ? "orange" : "orange",
                  bgcolor: activeTab === "Tạo tài khoản cho con" ? "#FFFBF3" : "white",
                  "&:hover": { bgcolor: "#FFFBF3", color: "orange" },
                }}
              >
                Tạo tài khoản cho con
            ...
            <Button
              fullWidth
              startIcon={<LogoutIcon />}
              sx={{ justifyContent: "flex-start", color: "red", textTransform: "none" }}
              onClick={logoutUser}
            >
              Đăng xuất
            </Button>
        ...
        {/* Right Content */}
        <Box sx={{ flex: 4, bgcolor: "white", padding: 3, boxShadow: 4, borderRadius: 2 }}>
          {activeTab === "Thông tin cá nhân" && (
            <Box>
              {isEditing ? (
                <Box>
                  ...
                  <TextField
                    ...
                    value={editableInfo.fullName}
                    onChange={(e) =>
                      setEditableInfo({ ...editableInfo, fullName: e.target.value })
                    }
                    sx={{ mb: 2 }}
                  />
                  <TextField
                    ...
                    value={editableInfo.dateOfBirth}
                    onChange={(e) =>
                      setEditableInfo({ ...editableInfo, dateOfBirth: e.target.value })
                    }
                    InputLabelProps={{ shrink: true }}
                    sx={{ mb: 2 }}
                  />
                  ...
                  <Button
                    ...
                    color="primary"
                    onClick={handleUpdateUser}
                    sx={{ textTransform: "none", color: "white", ":hover": { bgcolor: "#99BC4D" } }}
                  >
                    Cập nhật thông tin
                  </Button>
                </Box>
                ...
                <Box>
                  ...
                  <Box sx={{ display: "grid", gridTemplateColumns: "150px 1fr", gap: 2, pt: 3 }}>
                    <Typography>Họ tên:</Typography>
                    <Typography fontWeight="bold">{userInfo.fullName || "-"}</Typography>
                    <Typography>Tên đăng nhập:</Typography>
                    <Typography fontWeight="bold">{userInfo.username}</Typography>
                    <Typography>Loại tài khoản:</Typography>
                    <Typography fontWeight="bold">{userInfo.role || "-"}</Typography>
                    <Typography>Ngày sinh:</Typography>
                    <Typography fontWeight="bold">{userInfo.dateOfBirth || "-"}</Typography>
                    <Typography>Email:</Typography>
                    <Typography fontWeight="bold">{userInfo.email}</Typography>
                  </Box>
                </Box>
          ...
          {activeTab === "Lịch sử làm bài" && (
            <Box>
              <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2, justifyContent: "space-between",flex: 1 }}>
                <Typography fontSize={20} fontWeight="bold" color="#99BC4D" >
                  Lịch sử làm bài
                </Typography>
                <TextField
                  select
                  ...
                  sx={{ maxWidth: "200px" }}
                >
                  <MenuItem value="Tất cả môn học">Tất cả môn học</MenuItem>
                      ...
                      {quizHistory.length > 0 ? (
                        quizHistory.map((record, index) => (
                          <TableRow key={record.recordId}>
                            ...
                            <TableCell>{record.correctAnswers}</TableCell>
                            <TableCell>{record.points}</TableCell>
                            <TableCell>{record.timeSpentFormatted || formatTimeSpent(record.timeSpent)}</TableCell>
          ...
          {activeTab === "Tạo tài khoản cho con" && (
            <Box>
              <Typography fontSize={20} fontWeight="bold" mb={2} color="#99BC4D">
                Tạo tài khoản cho con
              </Typography>
              <Box display="flex" flexDirection="column">
                <Typography fontSize={14} color="#63A2DC" fontStyle="italic">
                ...
                <Box sx={{ display: "grid", marginY: 2, gap: 2 }}>
                  <Box>
                    <Typography fontSize={14} fontWeight="600">Họ và tên</Typography>
                    <TextField
                      label="Nhập họ và tên"
                      value={studentInfo.fullName}
                  ...
                  <Box>
                    <Typography fontSize={14} fontWeight="600">Tên đăng nhập</Typography>
                    <TextField
                      label="Nhập tên đăng nhập"
                      value={studentInfo.username}
                  ...
                  <Box>
                    <Typography fontSize={14} fontWeight="600">Mật khẩu</Typography>
                    <TextField
                      label="Nhập mật khẩu"
                      type="password"
                      value={studentInfo.password}
                  ...
                  <Box>
                    <Typography fontSize={14} fontWeight="600">Khối lớp</Typography>
                    <TextField
                      select
                      label="Chọn khối lớp"
                      value={studentInfo.grade}
          ...
          {activeTab === "Đổi mật khẩu" && (
            <Box>
              ...
              <Box display="flex" flexDirection="column">
                <Box sx={{ mb: 2 }}>
                  <Typography fontSize={14} fontWeight="600">Mật khẩu hiện tại</Typography>
                  <TextField
                    label="Nhập mật khẩu hiện tại"
                    type={showPassword ? "text" : "password"}
                    value={oldPassword}
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={togglePasswordVisibility}>
                            {showPassword ? (
                              <VisibilityOff fontSize="small" sx={{ color: "#99BC4D" }} />
                            ) : (
                              <Visibility fontSize="small" sx={{ color: "#99BC4D" }} />
                            )}
                          </IconButton>
                        </InputAdornment>
                      ),
                ...
                <Box sx={{ mb: 2 }}>
                  <Typography fontSize={14} fontWeight="600">Mật khẩu mới</Typography>
                  <TextField
                    label="Nhập mật khẩu mới"
                    type={showNewPassword ? "text" : "password"}
                    value={newPassword}
                ...
                <Box sx={{ mb: 2 }}>
                  <Typography fontSize={14} fontWeight="600">Xác nhận mật khẩu mới</Typography>
                  <TextField
                    label="Nhập lại mật khẩu mới"
                    type={showConfirmPassword ? "text" : "password"}
                    value={confirmNewPassword}
                    ...
                    helperText={errorConfirmPassword || undefined}
                    fullWidth
                    sx={{ ".MuiFormHelperText-root.Mui-error": { fontSize: "14px", paddingY: "2px" } }}
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={toggleConfirmPasswordVisibility}>
